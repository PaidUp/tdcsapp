...
describe('Cronjob workflow Delinquent', function (){
    this.timeout(15000);
    ...
    describe('Prepare user', function (){
        ...
        it('Create fake child to userFake', function(done){
            var userFake = {token:modelSpec.token,firstName:modelSpec.firstName + ' child',lastName:modelSpec.lastName + ' child', gender:modelSpec.gender};
            request(app)
            .post('/api/v1/user/create')
    ...
    describe('validate loan is delinquent', function (){
        this.timeout(60000);
        it('/loan validate', function(done) {
            ...
            loanService.findOne(filter,function(err,loan){
                ...
                assert.equal(loan.state, 'delinquent');
                assert.equal(loan.schedule[0].state, 'failed');
                done();
            });
        });
    });
